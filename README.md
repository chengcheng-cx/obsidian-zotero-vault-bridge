...
讓 PDF 實體檔留在 Obsidian Vault，由 Zotero 10 建立 linked attachment、執行原生 metadata recognition 並保存 citation key，再由 Obsidian 建立 Literature Note 與可點擊的引用連結；需要時也可切換成純 Pandoc citation。

目前已完成五個可安裝里程碑，包括引用連結、完整可靠性與發佈硬化：
...
7. 在任何 Markdown 筆記輸入 `[@`，即可搜尋 Zotero 並插入顯示為 `[@citationKey]` 的 Literature Note 連結。
...
- 使用 `↑`／`↓` 選擇、`Enter` 插入、`Esc` 關閉，也可直接用滑鼠選取。
- 搜尋本身不修改 Zotero；只有選取文獻時，才確認並保存該項目的 citation key。
- 預設插入真正的 Obsidian wikilink：`[[02_Literature/citationKey|[@citationKey]]]`；畫面仍顯示 `[@citationKey]`，點擊會開啟 Literature Note。
- 若 Literature Note 尚未建立，會改成顯示 `@citationKey` 的安全 `zotero://select` 連結，直接開啟 Zotero 文獻項目，避免建立會阻擋後續同步的空白筆記。
- 在 `Settings → Zotero Vault Bridge → Citation insertion format` 選擇 `Pandoc citation`，可恢復插入純文字 `[@citationKey]`。
...
- Autocomplete 目前以單一 `[@` trigger 為範圍；使用 Pandoc 模式時，多筆 citation cluster 可在插入後手動組合。
